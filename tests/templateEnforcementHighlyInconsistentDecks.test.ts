...
async function createOutputPath(fileName: string): Promise<string> {
  const workDir = await mkdtemp(path.join(tmpdir(), "pptx-fixer-enforcement-inconsistent-"));
...
test("safely applies admitted alignment enforcement on highly inconsistent template-driven decks", async () => {
  const candidatePath = path.resolve("testdata/corpus/fingerprint/aptos-template-anchor-inconsistent-alignment-drift.pptx");
  const outputPath = await createOutputPath("inconsistent-alignment-output.pptx");
  ...
  const result = await runTemplateEnforcementCore({
    ...
    templates: [
      {
        inputPath: path.resolve("testdata/corpus/fingerprint/aptos-template-anchor-inconsistent.pptx"),
  ...
  assert.equal(result.admittedTemplateDeckId, "aptos-template-anchor-inconsistent.pptx");
  assert.equal(result.admittedTemplateFamilyId, "aptos-left-inconsistent");
  ...
  assert.equal(outputAudit.fontSizeDrift.driftRuns.length, 2);
...
test("safely applies admitted font-family enforcement on highly inconsistent decks and leaves out-of-scope font size untouched", async () => {
  const candidatePath = path.resolve("testdata/corpus/fingerprint/aptos-template-anchor-inconsistent-font-family-drift.pptx");
  const outputPath = await createOutputPath("inconsistent-font-family-output.pptx");
...
test("blocks highly inconsistent enforcement when external template selection is ambiguous", async () => {
  const candidatePath = path.resolve("testdata/corpus/fingerprint/aptos-template-anchor-inconsistent-font-family-drift.pptx");
  const outputPath = await createOutputPath("inconsistent-ambiguous-output.pptx");
  ...
  const result = await runTemplateEnforcementCore({
    ...
    templates: [
      ...
      {
        inputPath: path.resolve("testdata/corpus/fingerprint/aptos-template-anchor-inconsistent-right-conflict.pptx"),
...
test("blocks highly inconsistent enforcement when only wrong-template anchors are available", async () => {
  const candidatePath = path.resolve("testdata/corpus/fingerprint/aptos-template-anchor-inconsistent-font-family-drift.pptx");
  const outputPath = await createOutputPath("inconsistent-rejected-output.pptx");
...
test("blocks admitted highly inconsistent alignment enforcement when out-of-scope spacing mutation would be required", async () => {
  const candidatePath = path.resolve("testdata/corpus/fingerprint/aptos-template-anchor-inconsistent-alignment-spacing-guard.pptx");
  const outputPath = await createOutputPath("inconsistent-guard-output.pptx");
  ...
  assert.equal(result.scopeDecision, "enforcementEligible");
  assert.deepEqual(result.appliedClasses, []);
  assert.deepEqual(result.blockedClasses, ["alignment"]);
  assert.ok(result.decisionReasons.includes("alignmentStageProducedOutOfScopeChanges"));
  assert.deepEqual(await readFile(candidatePath), await readFile(outputPath));
});

test("is deterministic across repeated highly inconsistent enforcement runs", async () => {
  const candidatePath = path.resolve("testdata/corpus/fingerprint/aptos-template-anchor-inconsistent-font-family-drift.pptx");
  const firstOutputPath = await createOutputPath("inconsistent-font-first-output.pptx");
  const secondOutputPath = await createOutputPath("inconsistent-font-second-output.pptx");
