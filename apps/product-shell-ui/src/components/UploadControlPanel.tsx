...
export function UploadControlPanel(props: UploadControlPanelProps) {
  return (
    <section className="flex h-full flex-col gap-4 rounded-[22px] border border-[#2a2a33] bg-[#141418] p-4 shadow-[0_24px_80px_rgba(0,0,0,0.28)]">
      <div className="rounded-[18px] border border-[#2a2a33] bg-[#1b1b21] p-4">
        ...
        <label className="mt-4 block cursor-pointer rounded-[16px] border border-dashed border-[#323541] bg-[#121218] px-4 py-4 transition hover:border-[#4a4d5b]">
          <div className="flex items-center justify-between gap-4">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs leading-5 text-[#9d9dad]">
                {props.file ? "Automatic audit runs after file selection." : "Use a source deck that needs normalization."}
              </p>
            </div>
            <span className="rounded-full border border-[#3a3f48] bg-[#20222b] px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#d7c4a1]">
              Browse
            </span>
          </div>
          <input
      ...
      <div className="rounded-[18px] border border-[#2a2a33] bg-[#1b1b21] p-4">
        ...
        >
          {props.isFixing ? "Fixing deck" : props.isAuditing ? "Waiting for audit" : "Fix deck"}
        </button>
      </div>
    </section>
  );
