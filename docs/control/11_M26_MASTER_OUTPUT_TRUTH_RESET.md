...
- Active sprint: `M26.19 - End-to-End Green Path Closure`
- Status: `DONE`
- Scope note: Make the real product-shell upload, audit, fix, result, and download path truthful and complete for the committed stress repro deck so the user-visible app flow matches the current engine-side green truth.
- Evidence note: `DONE on 2026-03-30: the product-shell now persists a real JSON report artifact next to the fixed PPTX and returns both download URLs from /fix. Browser validation on hostile/formatting-drift-stress-repro-v1.pptx shows Ready with category rows font 17 -> 0, size 11 -> 0, paragraph spacing 9 -> 0, bullet 13 -> 0, alignment 0 -> 0, and line spacing 11 -> 0; the downloaded JSON report matches that same noRemainingIssues truth, and a fresh audit of the downloaded fixed PPTX comes back all-zero on the tracked categories.`
...
- Evidence note: `DONE on 2026-03-29: packages/fix/runAllFixes.ts now refreshes audit truth between cleanup stages and runs a second paragraph-spacing pass when refreshed audit still exposes spacing drift. packages/fix/spacingFix.ts now converges mixed hostile/local explicit outliers safely while still preferring inherited baselines when they are the true local majority. packages/fix/lineSpacingFix.ts can clear fully explicit residual line-spacing shapes back to inherited leading. packages/fix/bulletFix.ts now converges anchored mixed-marker lists across marker kinds and synchronizes indentation to the anchored marker paragraph. packages/audit/pptxAudit.ts now distinguishes repeated non-left local roles, intro-plus-bullet cadence, and protected uniform local paragraph-spacing blocks from real unresolved paragraph-spacing drift. packages/fix/recommendedActionSummary.ts now treats zero remaining drift as post-cleanup review rather than stale manual attention. Real output proof on hostile/formatting-drift-stress-repro-v1.pptx is font 17 -> 0, font size 11 -> 0, paragraph spacing 9 -> 0, bullet drift 13 -> 0, alignment 0 -> 0, and line spacing 11 -> 0; canonical master improves paragraph spacing 5 -> 1, hostile chaos gate closes 3 -> 0, mixed hard boundary stays 3 -> 3 with zero paragraph-level changes, and combined QA closes 5 -> 0.`

### M26.19 - End-to-End Green Path Closure

- Status: `DONE`
- Problem statement: The engine-side stress deck is green, but the real product-shell upload, result, and artifact-download path still needs proof that it preserves that same truth for an actual user.
- Required runtime target: Product-shell fix response shaping, report/download handoff, and any minimal result-surface wiring needed so upload, fix, UI truth, JSON report truth, and fixed PPTX truth all agree on `hostile/formatting-drift-stress-repro-v1.pptx`.
- Required truth source: `testdata/corpus/hostile/formatting-drift-stress-repro-v1.pptx`, `testdata/corpus/master/cleandeck-master-acceptance-v1.pptx`, `testdata/corpus/hostile/cleandeck-chaos-gate-v1.pptx`, `testdata/corpus/boundary/mixed-hard-boundary-v1.pptx`, and `testdata/corpus/mixed-formatting/combined-qa-test-deck-v1.pptx`.
- Required boundary check: The user-visible app path must stay truthful without weakening validators or regressing protected decks.
- Completion condition: PASS only if the actual app flow shows the same green truth as the engine, downloadable artifacts are real and consistent, and protected decks do not regress materially.
- Evidence note: `DONE on 2026-03-30: `/fix` now emits both PPTX and JSON report download URLs, the product shell renders both as real links, and browser validation on the stress deck shows `ready / noRemainingIssues` with font 17 -> 0, size 11 -> 0, paragraph spacing 9 -> 0, bullet 13 -> 0, alignment 0 -> 0, and line spacing 11 -> 0. The downloaded JSON report matches the same readiness truth, and a fresh audit of the downloaded fixed PPTX comes back 0 font drift, 0 size drift, 0 paragraph spacing drift, 0 bullet drift, 0 alignment drift, and 0 line-spacing drift. Protected reruns stayed honest: master remains `mostlyReady` with paragraph spacing `5 -> 1`, hostile chaos gate stays `ready` with all tracked categories at `0`, mixed hard boundary stays `manualReviewRecommended` with paragraph spacing `3 -> 3` and line spacing `3 -> 3`, and combined QA stays `ready` with all tracked categories at `0`.`
