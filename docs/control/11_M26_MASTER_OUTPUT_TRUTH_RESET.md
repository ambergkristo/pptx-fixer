...
- Evidence note: `DONE on 2026-03-29: runAllFixes now re-audits between runtime stages, spacing normalization can safely converge hostile/local explicit outliers without flattening protected roles, line-spacing cleanup can clear fully explicit residual shapes back to inherited leading, bullet normalization can converge anchored mixed-marker lists across marker kinds, and paragraph-spacing audit now excludes repeated local non-left roles plus list-intro cadence from false residual counts. The committed stress repro now closes font 17 -> 0, font size 11 -> 0, paragraph spacing 9 -> 0, bullet drift 13 -> 0, alignment 0 -> 0, and line spacing 11 -> 0. Protected truth stayed honest: mixed hard boundary remains 3 -> 3 paragraph spacing and 3 -> 3 line spacing with zero runtime changes; canonical master improves paragraph spacing 5 -> 1 while other protected categories close; hostile chaos gate closes paragraph spacing 3 -> 0 without regressing already-closed categories; combined QA closes paragraph spacing 5 -> 0 while bullet, alignment, and line spacing all close to 0.`
